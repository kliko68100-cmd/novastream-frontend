...
function AnimeCard({ media, onClick }: { media: AniListMedia; onClick: () => void }) {
  ...
  return (
    <motion.div onClick={onClick} whileHover={{ scale: 1.04 }} transition={{ duration: 0.2 }}
      ...
      <p className="mt-2 text-xs font-semibold text-nova-text truncate px-0.5">{title}</p>
  ...
  return (
    <section className="mb-10">
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-none px-4 md:px-10 pb-2">
        {isLoading
          ? Array.from({length:8}).map((_,i) => <div key={i} className="shrink-0 w-36 md:w-44 aspect-[2/3] bg-nova-bg2 rounded-xl animate-pulse" />)
        ...
        <div className="relative h-[45vh] min-h-[280px] mb-10 overflow-hidden">
          ...
          <div className="absolute bottom-8 left-4 md:left-10 max-w-lg">
            ...
            <button onClick={() => goTo(hero)}
              className="px-6 py-2.5 bg-nova-accent rounded-full text-white font-bold hover:bg-nova-accent/90 transition-all shadow-lg shadow-nova-accent/30">
              ▶ Regarder
            </button>
          </div>
        </div>
      )}
      <Row title={`Saison ${seasonFr[season]} ${year}`} icon="🌸" items={seasonalData?.media??[]} onClickItem={goTo} isLoading={seasonalLoading} />
      <Row title="Tendances" icon="🔥" items={trendData?.media??[]} onClickItem={goTo} isLoading={trendLoading} />
      <Row title="Populaires" icon="⭐" items={popularData?.media??[]} onClickItem={goTo} isLoading={popularLoading} />
    </div>
  );
}
