                  ...
                  {searchData.media.slice(0, 6).map((item: any) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-nova-text truncate">{item.title?.userPreferred ?? item.title?.romaji ?? ''}</p>
                        <p className="text-xs text-nova-muted">{item.seasonYear ?? ''}</p>
                      </div>
...
export function BottomNav() {
  ...
  const tabs = [
    ...
    { to: '/manga',  icon: BookIcon,    label: 'Manga'     },
  ...
  if (location.pathname.startsWith('/watch') || location.pathname.includes('/read/')) return null;
...
function BookIcon({ className }: { className?: string }) {
  return <svg className={className} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>;
}
